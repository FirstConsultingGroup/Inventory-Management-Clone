...
// Debug function to inspect refresh token scopes
const debugTokenScopes = async (refreshToken: string) => {
  try {
    const res = await fetch(`https://oauth2.googleapis.com/tokeninfo?refresh_token=${refreshToken}`);
    const info = await res.json();
    log("info", "DEBUG: Refresh token scopes", info.scope);
  } catch (err) {
    log("error", "DEBUG: Failed to inspect token scope", err);
  }
};

serve(async (req) => {
  if (req.method === "OPTIONS") return makeRes({ ok: true });
  if (req.method !== "POST") return makeRes({ success: false, message: "Method not allowed" }, 405);
  ...
  if (!from || !company_id) {
    return makeRes({ success: false, message: "Missing required fields: from, company_id" }, 400);
  }
  ...
  if (dbError || !data) {
    ...
    return makeRes({ success: false, message: "Email not authenticated or token not found" }, 403);
  }

  const refreshToken = data.system_config_value;
  const dbRecordId = data.id;

  // DEBUG: Log granted scopes for this refresh token
  await debugTokenScopes(refreshToken);
  ...
  try {
    ...
    return makeRes({ success: false, message: "Email failed", error: String(topErr?.message ?? topErr) }, 500);
  }
});
