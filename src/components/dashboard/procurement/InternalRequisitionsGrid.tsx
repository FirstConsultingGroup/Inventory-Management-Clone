...
interface InternalRequisitionsGridProps {
    data: Requisition[];
...
export function InternalRequisitionsGrid({
    data,
    onAction,
    onBulkAction,
    statusFilter,
    onStatusFilterChange,
}: InternalRequisitionsGridProps) {
...
const [subModulePermissions, setSubModulePermissions] = useState<any[]>([]);
  ...
  const hasSubModulePermission = (subModuleName: string) => {
    ...
    return (
        <>
        {hasSubModulePermission('Purchase Requisitions - In-House')&&(
   <Card className="">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <div className="space-y-1">
                    <CardTitle className="text-xl text-gray-800 flex items-center gap-2">
                        <Building className="h-5 w-5 text-indigo-500" />
                        Purchase Requisitions - In-House
                    </CardTitle>
                    <CardDescription>Approved internal requisitions pending action</CardDescription>
                </div>
                        ...
                        <TableBody>
                            {paginatedData.length === 0 ? (
                                ...
                                paginatedData.map((row) => (
                                    <TableRow key={row.id} data-state={selectedRows.has(row.id) ? 'selected' : undefined} className="hover:bg-indigo-50/30 transition-colors">
                                        <TableCell>
                                            <Checkbox
                                        ...
                                        <TableCell className="font-medium text-indigo-600">{row.orderNumber}</TableCell>
