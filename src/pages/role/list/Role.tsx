...
type SortField = 'role_id' | 'name' | 'status' | 'created_at';
...
export const RoleMaster: React.FC = () => {
  ...
  // const isAdmin = userRole?.toLowerCase() === 'super admin' || userRole?.toLowerCase() === 'administrator';
   ...
   useEffect(() => {
      const fetchPermissions = async () => {
        if (userData?.id) {
          const res = await loadModulePermissions(appCode, 'Role Master', userData.id);
          console.log(res)
  ...
  const fetchRoles = useCallback(async () => {
    ...
    try {
      ...
      if (debouncedSearchQuery.trim()) {
        ...
        query = query.or(`role_id.ilike.%${s}%,name.ilike.%${s}%,description.ilike.%${s}%`);
      }
      ...
      const { data: rolesData, error, count } = await query;
      ...
      if (roleIds.length > 0) {
        ...
        userCounts = (usersData || []).reduce((acc, u) => {
      ...
      setRoles(rolesWithCount as Role[]);
  ...
  const handleDeleteRole = async () => {
    ...
    const isLocked = await checkEntityLock(roleToDelete.id); 
    if (isLocked) {
    ...
    try {
      ...
      if (users && users.length > 0) {
        toast.error('Cannot delete role. Users are assigned to this role.', { position: 'top-center' });
        setIsDeleteDialogOpen(false);
        setRoleToDelete(undefined);
        return;
      }
      ...
      const systemLogs = {
        ...
        scope: 'Delete',
        key: '',
        log: `Role: ${roleToDelete.name} (${roleToDelete.role_id}) deleted.`,
                ...
                operations: [
                    {
                        table: 'role_master',
                        type: 'delete',
                        match: { id: roleToDelete.id, company_id: userData?.company_id ?? userData?.id ?? '' }
                    },
                    {
                        table: 'system_log',
                        type: 'insert',
      ...
      toast.error(`Failed to delete role: ${err.message}`, { position: 'top-center' });
      setIsDeleteDialogOpen(false);
      setRoleToDelete(undefined);
    }
  };
  ...
  const exportRolesCSV = async () => {
    ...
    const fetchAllRoles = async (): Promise<Role[]> => {
      ...
      return data.map((r) => ({ ...r, users_count: userCounts[r.id] || 0 })) as Role[];
    };

    await exportSupabaseTableToCSV<Role>({
      reportTitle: 'Role Master Report',
      headers: ['Role ID', 'Role Name', 'Description', 'Status', 'Users Count', 'Created At'],
      rowMapper: (role) => [
        `"${role.role_id || ''}"`,
        `"${role.name || ''}"`,
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="mt-1">
                      Manage user roles and their access configurations
                    </CardDescription>
                  </div>
                </div>
                                  ...
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <div>
                                        <Button
                                          ...
                                          disabled={roles.length === 0 || loading || isExporting || !hasPermission('Export')}
                                        >
                                    ...
                                    <TooltipContent>
                                      {hasPermission('Export')
                                        ? 'Export CSV'
                                        : 'You do not have permission to export roles'}
                                    </TooltipContent>
                                  </Tooltip>
                    ...
                    {loading ? (
                      Array(itemsPerPage).fill(0).map((_, index) => (
                        <TableRow key={index} className="hover:bg-gray-50">
                          <TableCell className="py-3"><div className="h-6 w-24 bg-gray-200 rounded animate-pulse" /></TableCell>
                          <TableCell><div className="h-6 w-40 bg-gray-200 rounded animate-pulse" /></TableCell>
                          <TableCell><div className="h-6 w-60 bg-gray-200 rounded animate-pulse" /></TableCell>
                        ...
                        <TableCell colSpan={6} className="h-24 text-center text-muted-foreground">
                          <div className="flex flex-col items-center justify-center py-6">
                            ...
                            <p className="text-base font-medium">
                              {searchQuery.trim() ? 'No roles found matching your search' : 'No roles available'}
                            </p>
                      ...
                      roles.map((role) => (
                        <TableRow key={role.id} className="hover:bg-gray-50">
                          ...
                          <TableCell className="text-left">
                            <Badge
                              variant="outline"
                              className={`font-medium ${getStatusBadgeColor(role.status)}`}
                            >
                              {role.status ? 'Active' : 'Inactive'}
                            </Badge>
                          </TableCell>
                            ...
                            <div className="flex justify-center gap-2">
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <div>
                                    ...
                                    variant="outline"
                                    size="icon"
                                    onClick={() => navigate(`/dashboard/role-master/view/${role.id}`)}
                                ...
                                <TooltipTrigger asChild>
                                  <div>
                                    <Button
                                      variant="outline"
                                      size="icon"
                                      onClick={() => navigate(`/dashboard/role-master/edit/${role.id}`)}
                              ...
                              {role.users_count && role.users_count > 0 ? (
                                <Tooltip>
                                  ...
                                  <TooltipContent>
                                    <p>Cannot delete role. Users are assigned to this role.</p>
                                  </TooltipContent>
                                </Tooltip>
                              ) : (
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <div>
                                      <Button
                                        variant="outline"
                                        size="icon"
                                        className={`text-destructive hover:bg-destructive/10 ${!hasPermission('Delete') ? 'opacity-50 cursor-not-allowed' : ''}`}
                                        onClick={() => { setRoleToDelete(role); setIsDeleteDialogOpen(true); }}
                                        disabled={loading || !hasPermission('Delete')}
                                      >
                                        <Trash2 className="h-4 w-4" />
                                      </Button>
                                    </div>
                                  ...
                                  <TooltipContent>
                                    ...
                                    <p>You do not have permission to delete roles</p>
                                  ) : (
                                    <p>Delete Role</p>
                                  )}
                                  </TooltipContent>
          ...
          <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Are you sure you want to delete the role "{roleToDelete?.name}" ({roleToDelete?.role_id})?
                  This action cannot be undone.
                </DialogDescription>
              </DialogHeader>
              <DialogFooter className="flex justify-end gap-2">
                <DialogClose asChild>
                  <Button variant="outline" onClick={() => setRoleToDelete(undefined)} disabled={loading}>
                    Cancel
                  </Button>
                </DialogClose>
