import { useState, useEffect, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from '@/components/ui/table';
import {
  Card,
  CardContent,
  CardHeader,
  CardTitle,
  CardDescription,
} from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
  TooltipTrigger,
} from "@/components/ui/tooltip";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
  DialogClose,
} from '@/components/ui/dialog';
import {
  Plus,
  Edit,
  Eye,
  Trash2,
  Search,
  ChevronLeft,
  ChevronRight,
  Filter,
  MapPin,
  ArrowUpDown,
  ArrowUp,
  ArrowDown,
  Download,
} from 'lucide-react';
import { supabase } from '@/Utils/types/supabaseClient';
import { exportSupabaseTableToCSV } from '@/Utils/csvExport';
import toast from 'react-hot-toast';
import { Badge } from '@/components/ui/badge';
import { LocationMaster } from '@/types/inventory';
import { initiateApprovalRequest, loadModulePermissions, checkEntityLock } from '@/Utils/commonFun';
import { IUser } from '@/Utils/constants';

type SortField = 'location_id' | 'location_name' | 'is_active' | 'created_at';
type SortDirection = 'asc' | 'desc' | null;

interface SortConfig {
  field: SortField | null;
  direction: SortDirection;
}

type LocationWithUsage = LocationMaster & {
  used_in_store?: boolean;
  used_in_users?: boolean;
};

export function LocationManagement() {
  const navigate = useNavigate();
  const [searchQuery, setSearchQuery] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(10);
  const [locations, setLocations] = useState<LocationWithUsage[]>([]);
  const [totalItems, setTotalItems] = useState(0);
  const [totalPages, setTotalPages] = useState(0);
  const [statusFilter, setStatusFilter] = useState('all');
  const [loading, setLoading] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [locationToDelete, setLocationToDelete] = useState<LocationMaster>();
  const [sortConfig, setSortConfig] = useState<SortConfig>({
    field: 'created_at',
    direction: 'desc',
  });
  const [isExporting, setIsExporting] = useState(false);
  const [modulePermissions, setModulePermissions] = useState<any[]>([]);

  // Get company_id from localStorage
  const user = localStorage.getItem("userData");
  const userData: IUser | null = user ? JSON.parse(user) : null;
  const company_id = userData?.company_id || null;
  const user_id = userData?.id || null;
  const appCode = import.meta.env.VITE_APP_AUTH_CODE || 'INV-001';


  // Status filters
  const statusFilters = [
    { value: 'all', label: 'All Status' },
    { value: 'active', label: 'Active' },
    { value: 'inactive', label: 'Inactive' }
  ];

  // Debounce search
  const [debouncedSearchQuery, setDebouncedSearchQuery] = useState('');

  useEffect(() => {
    const timer = setTimeout(() => {
      setDebouncedSearchQuery(searchQuery);
    }, 500);
    return () => clearTimeout(timer);
  }, [searchQuery]);

  useEffect(() => {
    const fetchPermissions = async () => {
      if (userData?.id) {
        const res = await loadModulePermissions(appCode, 'Location Master', userData.id);
        console.log(res)
        if (res && res.permissions) {
          setModulePermissions(res.permissions);
        }
      }
    };
    fetchPermissions();
  }, [userData?.id, userData?.role_id]);

  const hasPermission = (actionName: string) => {
    const perm = modulePermissions.find((p: any) => p.action_id?.actionName?.toLowerCase() === actionName.toLowerCase());
    return perm ? perm.isAllowed : false;
  };

  // Sort function
  const handleSort = (field: SortField) => {
    let direction: SortDirection = 'asc';
    if (sortConfig.field === field) {
      if (sortConfig.direction === 'asc') {
        direction = 'desc';
      } else if (sortConfig.direction === 'desc') {
        direction = null;
      } else {
        direction = 'asc';
      }
    }
    setSortConfig({ field: direction ? field : null, direction });
    setCurrentPage(1);
  };

  const getSortIcon = (field: SortField) => {
    if (sortConfig.field !== field) {
      return <ArrowUpDown className="h-4 w-4 text-gray-400" />;
    }
    if (sortConfig.direction === 'asc') {
      return <ArrowUp className="h-4 w-4 text-blue-600" />;
    } else if (sortConfig.direction === 'desc') {
      return <ArrowDown className="h-4 w-4 text-blue-600" />;
    }
    return <ArrowUpDown className="h-4 w-4 text-gray-400" />;
  };

  const checkLocationUsage = async (locationId: string) => {
    try {
      // Check store usage
      const { data: storeData, error: storeError } = await supabase
        .from('store_mgmt')
        .select('id')
        .eq('location_id', locationId)
        .limit(1);

      if (storeError) throw storeError;

      // Check user usage
      const { data: userData, error: userError } = await supabase
        .from('user_mgmt')
        .select('id')
        .filter('locations', 'cs', JSON.stringify([locationId]))
        .limit(1);

      if (userError) throw userError;

      const { data: employeeLocationData, error: employeeLocationError } =
  await supabase
    .from('user_mgmt')
    .select('id')
    .eq('employee_location_id', locationId)
    .limit(1);

if (employeeLocationError) throw employeeLocationError;

      return {
        used_in_store: storeData.length > 0,
        used_in_users: userData.length > 0 ||
    employeeLocationData.length > 0,
      };
    } catch (error) {
      console.error('Location usage check failed:', error);
      return {
        used_in_store: false,
        used_in_users: false,
      };
    }
  };

  // Fetch locations
  const fetchLocations = useCallback(async () => {
    if (!company_id) return;

    setLoading(true);

    try {
      let query = supabase
  .from('location_master')
  .select('*', { count: 'exact' })
  .eq('company_id', company_id)
   .eq('is_active', true); 

      // Apply search filter
      if (debouncedSearchQuery.trim()) {
        const searchLower = debouncedSearchQuery.toLowerCase();
        query = query.or(`location_id.ilike.%${searchLower}%,location_name.ilike.%${searchLower}%,additional_info.ilike.%${searchLower}%`);
      }

      // Apply status filter
      if (statusFilter !== 'all') {
  query = query.eq('status', statusFilter === 'active');
}
console.log("statusFilter", statusFilter);
      // Apply sorting
      if (sortConfig.field && sortConfig.direction) {
        query = query.order(sortConfig.field, { ascending: sortConfig.direction === 'asc' });
      }

      // Apply pagination
      const startIndex = (currentPage - 1) * itemsPerPage;
      query = query.range(startIndex, startIndex + itemsPerPage - 1);

      // Execute query
      const { data, error, count } = await query;

      if (error) throw error;

      // Filter out any records with null IDs
      const validLocations = (data || []).filter(location => location.id !== null) as LocationMaster[];

      const enrichedLocations: LocationWithUsage[] = await Promise.all(
        validLocations.map(async (loc) => {
          const usage = await checkLocationUsage(loc.id);
          return {
            ...loc,
            ...usage,
          };
        })
      );

      setLocations(enrichedLocations);
      setTotalItems(count || 0);
      setTotalPages(Math.ceil((count || 0) / itemsPerPage));
    } catch (error) {
      console.error('Error fetching locations:', error);
      toast.error('Failed to load locations', { position: 'top-center' });
    } finally {
      setLoading(false);
    }
  }, [currentPage, debouncedSearchQuery, statusFilter, itemsPerPage, sortConfig, company_id]);

  useEffect(() => {
    fetchLocations();
  }, [fetchLocations]);

  
const handleDeleteLocation = async () => {
  if (!locationToDelete) return;

    const isLocked = await checkEntityLock(locationToDelete.id);
    if (isLocked) {
      toast.error("This record is currently locked because it has a pending approval request.", { position: "top-center" });
      return;
    }

  if (!company_id || !user_id) {
    toast.error('User or company information not found. Please login again.', {
      position: 'top-center',
    });
    return;
  }

  try {
    
    const usage = await checkLocationUsage(locationToDelete.id);

    if (usage.used_in_store || usage.used_in_users) {
      toast.error(
        'Location is linked with users or stores and cannot be deleted',
        {
          position: 'top-center',
        }
      );
      return;
    }

   
    const systemLog = {
      company_id: company_id,
      transaction_date: new Date().toISOString(),
      module: 'Location Master',
      scope: 'Delete',
      key: locationToDelete.location_id,
      log: `Location: ${locationToDelete.location_name} (${locationToDelete.location_id}) deleted.`,
      action_by: user_id,
      created_at: new Date().toISOString(),
    };

    
    const action_payload = {
      operations: [
        {
          table: 'location_master',
          type: 'delete',
          match: {
            id: locationToDelete.id,
            company_id: company_id,
          },
        },
        {
          table: 'system_log',
          type: 'insert',
          data: systemLog,
        },
      ],
    };

    
    const approvalResponse = await initiateApprovalRequest({
      module_name: 'Location Master',
      action_name: 'Delete',
      company_id: company_id,
      requested_by: user_id,
      action_payload,
      entity_id: locationToDelete.id,
    });

    if (!approvalResponse?.success) {
      throw new Error(
        approvalResponse?.message || 'Approval initiation failed'
      );
    }

    
    if (approvalResponse.requires_approval) {
      toast.success(
        'Your action has been submitted and is currently pending approval.',
        {
          position: 'top-center',
        }
      );

      setIsDeleteDialogOpen(false);
      setLocationToDelete(undefined);

      return;
    }

    
    const { error: deleteError } = await supabase
      .from('location_master')
      .delete()
      .eq('id', locationToDelete.id)
      .eq('company_id', company_id);

    if (deleteError) {
      throw deleteError;
    }

    
    const { error: systemLogError } = await supabase
      .from('system_log')
      .insert(systemLog);

    if (systemLogError) {
      throw systemLogError;
    }

    
    toast.success('Location deleted successfully', {
      position: 'top-center',
    });

    setIsDeleteDialogOpen(false);
    setLocationToDelete(undefined);

   
    fetchLocations();

  } catch (error: any) {
    console.error('Error deleting location:', error);

    toast.error(
      `Failed to delete location: ${
        error?.message || 'Unknown error'
      }`,
      {
        position: 'top-center',
      }
    );

    setIsDeleteDialogOpen(false);
    setLocationToDelete(undefined);
  }
};



  const handleFilterReset = () => {
    setSearchQuery('');
    setStatusFilter('all');
    setItemsPerPage(10);
    setCurrentPage(1);
    setSortConfig({ field: 'created_at', direction: 'desc' });
    toast.success('Filters cleared successfully!', { position: 'top-center' });
  };

  // CSV export function
  const exportLocationsCSV = async () => {
    setIsExporting(true);

    const fetchAllLocationsForExport = async (): Promise<LocationMaster[]> => {
      try {
        const { data, error } = await supabase
          .from('location_master')
          .select('*')
          .eq('company_id', company_id!)
          .eq('is_active', true)
          .order('created_at', { ascending: false });

        if (error) throw error;

        return (data || []).filter(location => location.id !== null) as LocationMaster[];
      } catch (error) {
        throw error;
      }
    };

    await exportSupabaseTableToCSV<LocationMaster>({
      reportTitle: 'Location Master Report',
      headers: [
        'Location ID',
        'Location Name',
        'Status',
        'Additional Info',
        'Created At',
      ],
      rowMapper: (location) => [
        `"${location.location_id}"`,
        `"${location.location_name}"`,
        `"${isActive(location.is_active) ? 'Active' : 'Inactive'}"`,
        `"${location.additional_info || 'N/A'}"`,
        `"${new Date(location.created_at).toLocaleDateString('en-US', {
          year: 'numeric',
          month: 'short',
          day: '2-digit',
          hour: '2-digit',
          minute: '2-digit',
        })}"`,
      ],
      supabaseClient: supabase,
      fetcher: fetchAllLocationsForExport,
      onError: (error) => {
        console.error('Export error:', error);
        toast.error(`Failed to export locations: ${error.message}`, { position: 'top-center' });
      },
    });

    setIsExporting(false);
  };

  const formatDate = (dateString: string): string => {
    try {
      return new Date(dateString).toLocaleDateString('en-US', {
        year: 'numeric',
        month: 'short',
        day: '2-digit',
      });
    } catch {
      return 'Invalid Date';
    }
  };

  const getStatusBadgeColor = (status: boolean | null) => {
    return status === true
      ? 'bg-green-100 text-green-800 border-green-300'
      : 'bg-red-100 text-red-800 border-red-300';
  };

  const isActive = (status: boolean | null): boolean => {
    return status === true;
  };

  const isLocationDeletable = (location: LocationWithUsage) => {
    return !location.used_in_store && !location.used_in_users;
  };

  return (
    <TooltipProvider>
      <div className="p-6">
        <div className="mx-auto max-w-7xl space-y-6">
          <Card className="min-h-[85vh] shadow-sm">
            <CardHeader className="rounded-t-lg border-b pb-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center space-x-3">
                  <div className="p-2.5 rounded-lg bg-blue-100 shadow-sm">
                    <MapPin className="h-6 w-6 text-blue-600" />
                  </div>
                  <div>
                    <CardTitle className="text-2xl font-bold flex items-center gap-2">
                      Location Master
                    </CardTitle>
                    <CardDescription className="mt-1">
                      Manage and configure locations for your organization
                    </CardDescription>
                  </div>
                </div>
                <div className="flex gap-2">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div>
                        <Button
                          variant="outline"
                          onClick={exportLocationsCSV}
                          className="transition-colors"
                          disabled={locations.length === 0 || loading || isExporting || !hasPermission('Export')}
                        >
                          <Download className={`mr-2 h-4 w-4 ${isExporting ? 'animate-spin' : ''}`} />
                          <span>{isExporting ? 'Exporting...' : 'Export CSV'}</span>
                        </Button>
                      </div>
                    </TooltipTrigger>

                    <TooltipContent>
                      {hasPermission('Export')
                        ? 'Export CSV'
                        : 'You do not have permission to export locations'}
                    </TooltipContent>
                  </Tooltip>

                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div>
                        <Button
                          onClick={() => navigate('/dashboard/location-master/add')}
                          className="transition-colors"
                          disabled={loading || !hasPermission('Add')}
                        >
                          <Plus className="mr-2 h-4 w-4" />
                          Add Location
                        </Button>
                      </div>
                    </TooltipTrigger>

                    <TooltipContent>
                      {hasPermission('Add')
                        ? 'Add Location'
                        : 'You do not have permission to add locations'}
                    </TooltipContent>
                  </Tooltip>
                </div>
              </div>
            </CardHeader>

            <CardContent className="pt-6">
              {/* Filters */}
              <div className="mb-6">
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  <div className="relative flex-1 w-full sm:w-1/3">
                    <Search className="absolute left-3 top-3 h-4 w-4 text-gray-400" />
                    <Input
                      placeholder="Search by ID, name, or info..."
                      value={searchQuery}
                      onChange={(e) => {
                        setSearchQuery(e.target.value);
                        setCurrentPage(1);
                      }}
                      className="pl-10"
                    />
                  </div>
                  <div className="flex items-center gap-2 w-full sm:w-[180px]">
                    <Filter className="h-4 w-4 text-gray-500" />
                    <Select
                      value={statusFilter}
                      onValueChange={(value) => {
                         console.log("selected", value);
                        setStatusFilter(value);
                        setCurrentPage(1);
                      }}
                      disabled={loading}
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder="Filter by status" />
                      </SelectTrigger>
                      <SelectContent>
                        {statusFilters.map(filter => (
                          <SelectItem key={filter.value} value={filter.value}>
                            {filter.label}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  </div>
                  <Button
                    variant="outline"
                    onClick={handleFilterReset}
                    className="transition-colors w-full sm:w-auto"
                    disabled={loading}
                  >
                    Clear Filters
                  </Button>
                </div>
              </div>

              {/* Table */}
              <div className="rounded-lg overflow-hidden border shadow-sm">
                <Table>
                  <TableHeader>
                    <TableRow className="hover:bg-gray-50 border-gray-200">
                      <TableHead className="font-semibold w-[150px]">
                        <button
                          type="button"
                          onClick={() => handleSort('location_id')}
                          className="h-8 flex items-center gap-1 font-semibold cursor-pointer w-auto hover:text-blue-600 ps-2"
                        >
                          Location ID
                          {getSortIcon('location_id')}
                        </button>
                      </TableHead>
                      <TableHead className="font-semibold">
                        <button
                          type="button"
                          onClick={() => handleSort('location_name')}
                          className="h-8 flex items-center gap-1 font-semibold cursor-pointer w-auto hover:text-blue-600"
                        >
                          Location Name
                          {getSortIcon('location_name')}
                        </button>
                      </TableHead>
                      <TableHead className="font-semibold">
                        <button
                          type="button"
                          onClick={() => handleSort('is_active')}
                          className="h-8 flex items-center gap-1 font-semibold cursor-pointer w-full justify-start hover:text-blue-600"
                        >
                          Status
                          {getSortIcon('is_active')}
                        </button>
                      </TableHead>
                      <TableHead className="font-semibold">Info</TableHead>
                      <TableHead className="font-semibold">
                        <button
                          type="button"
                          onClick={() => handleSort('created_at')}
                          className="h-8 flex items-center gap-1 font-semibold cursor-pointer w-full justify-start hover:text-blue-600"
                        >
                          Created At
                          {getSortIcon('created_at')}
                        </button>
                      </TableHead>
                      <TableHead className="text-center font-semibold">Actions</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {loading ? (
                      Array(itemsPerPage).fill(0).map((_, index) => (
                        <TableRow key={index} className="hover:bg-gray-50">
                          <TableCell className="py-3"><div className="h-6 w-24 bg-gray-200 rounded animate-pulse"></div></TableCell>
                          <TableCell><div className="h-6 w-40 bg-gray-200 rounded animate-pulse"></div></TableCell>
                          <TableCell><div className="h-6 w-20 bg-gray-200 rounded animate-pulse"></div></TableCell>
                          <TableCell><div className="h-6 w-60 bg-gray-200 rounded animate-pulse"></div></TableCell>
                          <TableCell><div className="h-6 w-24 bg-gray-200 rounded animate-pulse"></div></TableCell>
                          <TableCell className="text-center">
                            <div className="flex justify-center gap-2">
                              <div className="h-8 w-8 bg-gray-200 rounded animate-pulse"></div>
                              <div className="h-8 w-8 bg-gray-200 rounded animate-pulse"></div>
                              <div className="h-8 w-8 bg-gray-200 rounded animate-pulse"></div>
                            </div>
                          </TableCell>
                        </TableRow>
                      ))
                    ) : locations.length === 0 ? (
                      <TableRow>
                        <TableCell
                          colSpan={6}
                          className="h-24 text-center text-muted-foreground"
                        >
                          <div className="flex flex-col items-center justify-center py-6">
                            <MapPin className="h-12 w-12 text-gray-300 mb-2" />
                            <p className="text-base font-medium">
                              {searchQuery.trim()
                                ? 'No locations found matching your search'
                                : 'No locations available'}
                            </p>
                            <p className="text-sm text-gray-500">
                              {searchQuery.trim()
                                ? 'Try adjusting your search terms or filters'
                                : 'Create a new location to get started'}
                            </p>
                          </div>
                        </TableCell>
                      </TableRow>
                    ) : (
                      locations.map((location) => (
                        <TableRow key={location.id} className="hover:bg-gray-50">
                          <TableCell className="font-medium py-3">
                            <div className="ps-2">
                              <p className="font-mono text-sm">{location.location_id}</p>
                            </div>
                          </TableCell>
                          <TableCell className="font-medium">
                            <p>{location.location_name}</p>
                          </TableCell>
                          <TableCell className="text-left">
                            <Badge
                              variant="outline"
                              className={`font-medium ${getStatusBadgeColor(location.status)}`}
                            >
                              {isActive(location.status) ? 'Active' : 'Inactive'}
                            </Badge>
                          </TableCell>
                          <TableCell className="min-w-[250px] whitespace-normal break-words">
                            <div className="max-w-md">
                              <p className="text-sm text-gray-600">
                                {location.additional_info || (
                                  <span className="text-gray-400 italic">No information provided</span>
                                )}
                              </p>
                            </div>
                          </TableCell>
                          <TableCell className="text-left">
                            <p className="text-sm">{formatDate(location.created_at)}</p>
                          </TableCell>
                          <TableCell className="text-center">
                            <div className="flex justify-center gap-2">
                              {/* View */}
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <div>
                                    <Button
                                      variant="outline"
                                      size="icon"
                                      onClick={() => navigate(`/dashboard/location-master/view/${location.id}`)}
                                      disabled={loading || !hasPermission('View')}
                                      className={!hasPermission('View') ? 'opacity-50 cursor-not-allowed' : ''}
                                    >
                                      <Eye className="h-4 w-4" />
                                    </Button>
                                  </div>
                                </TooltipTrigger>
                                <TooltipContent>
                                  {hasPermission('View')
                                    ? 'View Location'
                                    : 'You do not have permission to view locations'}
                                </TooltipContent>
                              </Tooltip>

                              {/* Edit */}
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <div>
                                    <Button
                                      variant="outline"
                                      size="icon"
                                      onClick={() => navigate(`/dashboard/location-master/edit/${location.id}`)}
                                      disabled={loading || !hasPermission('Edit')}
                                      className={!hasPermission('Edit') ? 'opacity-50 cursor-not-allowed' : ''}
                                    >
                                      <Edit className="h-4 w-4" />
                                    </Button>
                                  </div>
                                </TooltipTrigger>
                                <TooltipContent>
                                  {hasPermission('Edit')
                                    ? 'Edit Location'
                                    : 'You do not have permission to edit locations'}
                                </TooltipContent>
                              </Tooltip>

                              {/* Delete */}
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <span>
                                    <Button
                                      variant="outline"
                                      size="icon"
                                      className={`text-destructive hover:bg-destructive/10 ${(!hasPermission('Delete') || (!isLocationDeletable(location)))
                                        ? 'opacity-50 cursor-not-allowed'
                                        : ''
                                        }`}

                                      onClick={() => {
                                        if (!hasPermission('Delete')) return;

                                        if (isLocationDeletable(location)) {
                                          setLocationToDelete(location);
                                          setIsDeleteDialogOpen(true);
                                        }
                                      }}
                                      disabled={loading || !isLocationDeletable(location) || !hasPermission('Delete')}
                                    >
                                      <Trash2 className="h-4 w-4" />
                                    </Button>
                                  </span>
                                </TooltipTrigger>
                                <TooltipContent>
                                  {!hasPermission('Delete') ? (
                                    <p>You do not have permission to delete locations.</p>
                                  ) : !isLocationDeletable(location) ? (
                                    <p>Location is linked with users or stores</p>
                                  ) : (
                                    <p>Delete Location</p>
                                  )}
                                </TooltipContent>
                              </Tooltip>
                            </div>
                          </TableCell>
                        </TableRow>
                      ))
                    )}
                  </TableBody>
                </Table>
              </div>

              {/* Pagination */}
              <div className="flex flex-col sm:flex-row items-center justify-between px-2 py-6 gap-4">
                <div className="flex items-center gap-2">
                  <p className="text-sm text-muted-foreground">Show</p>
                  <Select
                    value={itemsPerPage.toString()}
                    onValueChange={(value) => {
                      setItemsPerPage(Number(value));
                      setCurrentPage(1);
                    }}
                    disabled={loading}
                  >
                    <SelectTrigger className="w-[70px]">
                      <SelectValue placeholder={itemsPerPage.toString()} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="5">5</SelectItem>
                      <SelectItem value="10">10</SelectItem>
                      <SelectItem value="20">20</SelectItem>
                      <SelectItem value="50">50</SelectItem>
                    </SelectContent>
                  </Select>
                  <p className="text-sm text-muted-foreground">entries</p>
                </div>

                <div className="flex items-center gap-2">
                  <p className="text-sm text-muted-foreground hidden sm:block">
                    Showing {locations.length > 0 ? ((currentPage - 1) * itemsPerPage) + 1 : 0} to {Math.min(currentPage * itemsPerPage, totalItems)} of {totalItems} entries
                  </p>
                  <div className="flex items-center space-x-2">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
                      disabled={currentPage === 1 || loading}
                    >
                      <ChevronLeft className="h-4 w-4 mr-1" />
                      Previous
                    </Button>
                    <div className="flex items-center justify-center text-sm font-medium bg-gray-100 px-3 py-1 rounded">
                      Page {currentPage} of {totalPages || 1}
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages || 1))}
                      disabled={currentPage === totalPages || loading || totalPages === 0}
                    >
                      Next
                      <ChevronRight className="h-4 w-4 ml-1" />
                    </Button>
                  </div>
                </div>
              </div>
            </CardContent>
          </Card>

          {/* Delete Confirmation Dialog */}
          <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle>Confirm Deletion</DialogTitle>
                <DialogDescription>
                  Are you sure you want to delete the location "{locationToDelete?.location_name}" ({locationToDelete?.location_id})? This action cannot be undone.
                </DialogDescription>
              </DialogHeader>
              <DialogFooter className="flex justify-end gap-2">
                <DialogClose asChild>
                  <Button variant="outline" onClick={() => setLocationToDelete(undefined)} disabled={loading}>
                    Cancel
                  </Button>
                </DialogClose>
                <Button
                  variant="destructive"
                  onClick={handleDeleteLocation}
                  disabled={loading}
                >
                  Delete
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
      </div>
    </TooltipProvider>
  );
}